...
export const getCategoriesFromUser = async function ()
{
	...
	const categories = await categoriesCollection
		.find({ $or: [{ author: new ObjectId(user.userId), active: true }, { author: new ObjectId(user.userId), active: undefined }] })
		.sort()
		.toArray();
...
export const deactivateCategory = async function (formData)
{
	await validateSession();
	const user = await getUserFromCookie();
	const errors = {};
	...
	await categoriesCollection.updateOne({ _id: ObjectId.createFromHexString(categoryId) }, { $set: { active: false } });
